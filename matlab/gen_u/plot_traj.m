function plot_traj(x0, v, p, P, q, Q, T, B, C, G)
    n = 20;
    dim = size(x0, 1);
    L = get_l(n);
    ...
    plot(ax_1, x_op_first_proj(1, :), x_op_first_proj(2, :), 'color', 'b', 'LineWidth', 2);
    plot(ax_2, x_op_second_proj(1, :), x_op_second_proj(2, :), 'color', 'b', 'LineWidth', 2);
    hold(ax_1, 'on');
    hold(ax_2, 'on');
...
%    x(dim + 1, :) = flip(T, 2);
    T = flip(T, 2);
    x0 = inv(G(T(1))) * x0;
    plot(ax_1, x0(1), x0(2), 'Color', 'r', 'Marker', 'o');
    plot(ax_2, x0(3), x0(4), 'Color', 'r', 'Marker', 'o');
    for i = 1 : size(x, 2)
       x(:,i) = inv(G(T(i))) * x(:, i);
    end
    plot(ax_1, x(1,:), x(2, :), 'Color', 'r');
    plot(ax_2, x(3,:), x(4, :), 'Color', 'r');
